...
var emotions = ["Anger", "Fear", "Sadness", "Joy", "Surprise", "Trust", "Anticipation"];
...
function makeButtons(){ 
	...
	for (var i = 0; i < emotions.length; i++){
		...
		var a = $('<button>'); 
		a.addClass('emotion'); // add a class
		a.attr('data-name', emotions[i]); // add a data-attribute
		a.text(emotions[i]); // make button text
		$('#buttonsView').append(a); // append the button to buttonsView div
	}
}

// handles addEmotion button event
$("#addEmotion").on("click", function(){

	// grabs the user emotion input
...
function displayGifs(){
	...
	var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + emotion + "&limit=9&api_key=dc6zaTOxFJmzC";
